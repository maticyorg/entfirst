  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <p className="text-sm font-mono text-brand tracking-wide">Verticals</p>
        <h2 className="mt-3 text-3xl sm:text-4xl font-semibold tracking-tight max-w-2xl">
          AI-powered assets, deployed across markets
        </h2>
        <p className="mt-4 max-w-2xl text-muted-foreground text-lg">
          Same engine, different verticals. Each one configured and deployed as a managed service\u2014not a product you maintain.
        </p>
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {verticals.map((v) => (
            <article
              key={v.name}
              className={`group relative rounded-2xl border border-white/10 bg-gradient-to-br ${v.color} p-6 transition-all hover:border-brand/30 hover:shadow-[0_0_40px_rgba(34,197,94,0.08)]`}
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h3 className="text-lg font-semibold font-mono">{v.name}</h3>
